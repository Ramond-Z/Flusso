...
- `flusso ls`：查看任务队列。
- `flusso delete JOB_ID`：删除尚未被调度的任务。
...
`flusso delete JOB_ID` 只允许删除尚未被调度的任务，也就是状态为 `PENDING` 或 `HELD` 的任务。已经进入 `RUNNING`、`SUCCEEDED` 或 `FAILED` 的任务不会被删除。
