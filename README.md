...
- `flusso submit --after JOB_ID`：提交必须等待上游任务成功后才可调度的任务。
...
```bash
uv run flusso submit --gpus 1 --name eval-demo --after 1 -- python eval.py --ckpt outputs/demo/latest.pt
```
...
`flusso submit --after JOB_ID` 会记录顺序依赖。依赖不存在时提交会失败；依赖存在时，子任务只有在所有上游任务状态都是 `SUCCEEDED` 后才会被调度。如果任一上游任务进入 `FAILED`、`CANCELLED` 或 `SKIPPED`，下游 `PENDING` 任务会被标记为 `SKIPPED`。
